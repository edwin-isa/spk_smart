                                    ...
                                    @foreach ($kriteria as $index => $kri)
                                        <tr class="border-b transition duration-300 ease-in-out hover:bg-neutral-100 dark:border-neutral-500 dark:hover:bg-neutral-600">
                                            ...
                                            <td class="whitespace-nowrap px-6 py-4">
                                                <form id="deleteForm{{ $index }}" action="{{ route('kriteria.destroy', $kri->id) }}" method="POST">
                                                    <a href="{{ route('kriteria.edit', $kri->id) }}"
                                                        class="btn btn-edit px-6 py-2.5 border border-blue-500 hover:bg-blue-300">
                                                        Edit
                                                    </a>
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit"
                                                        class="btn btn-delete px-6 py-2.5 border border-red-600 hover:bg-red-300 ml-6">
                                                        Hapus
                                                    </button>
                                                </form>
                                            </td>
